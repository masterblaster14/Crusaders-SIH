...
// **UPDATED** badges array with bonus points
const badges = [
...
// **NEW** function to check for and award badge bonuses
const checkAndAwardBadgeBonus = (oldCoins, newCoins) => {
    let bonusCoins = 0;
    let newBadgeEarned = null;

    for (const badge of badges) {
        if (oldCoins < badge.threshold && newCoins >= badge.threshold) {
            bonusCoins += badge.bonus; // Use the specific bonus from the badge object
    ...
    if (newBadgeEarned) {
        setTimeout(() => {
             alert(`Congratulations! You've earned the "${newBadgeEarned.name}" badge ${newBadgeEarned.icon}\n\nYou've been awarded a bonus of ${bonusCoins} Green Coins!`);
        }, 1600); 
    }
...
const renderChallenges = () => {
    ...
    challengesData.forEach(challenge => {
        ...
        card.innerHTML = `
            ...
            <div class="flex-shrink-0 text-center"><span class="block font-bold text-lg text-yellow-400">+${challenge.reward} GC</span><button class="mt-1 text-sm bg-gray-700 text-white font-bold py-2 px-4 rounded-sm transition-all duration-200 ease-in-out hover:bg-gray-600 submit-challenge-btn">Submit</button></div>
        `;
...
const renderProfile = () => {
    ...
    const earnedBadges = badges.filter(badge => userData.greenCoins >= badge.threshold);
...
const handleImageAnalysis = async () => {
    if (!uploadedImageData) return alert("Please upload an image first.");
    ...
    try {
        const response = await fetch(CLOUD_FUNCTION_URL, {
            ...
            body: JSON.stringify({
                ...
                mimeType: imageUploadInput.files[0].type
            }),
        });
        if (!response.ok) throw new Error(`Error from server: ${response.statusText}`);
        const data = await response.json();
        analysisResult.innerHTML = `<h3 class="text-2xl font-bold text-white mb-2">${data.wasteType}</h3><p class="text-sm text-gray-400 mb-4">Material: <span class="font-semibold">${data.material}</span></p><div class="border-t border-gray-600 pt-4"><p class="font-bold text-emerald-400 mb-2">Disposal Instructions:</p><p class="text-gray-300">${data.disposalInstructions}</p></div>`;
    } catch (error) {
        console.error("Analysis failed:", error);
        analysisResult.innerHTML = `<p class="text-red-500">Sorry, the analysis failed. Please try again.</p>`;
    } finally {
...
//          QUIZ LOGIC
// =======================================
// **UPDATED** function to correctly call the Gemini API
const fetchQuizQuestions = async (topic) => {
    const prompt = `
        Create a unique set of 5 multiple-choice quiz questions about "${topic}".
        The questions should be suitable for a university student.
...
// **UPDATED** endQuiz function to include the badge bonus logic
const endQuiz = async () => {
    const coinsFromQuiz = score * 10;
    const oldTotal = userData.greenCoins;

    const bonusFromBadges = checkAndAwardBadgeBonus(oldTotal, oldTotal + coinsFromQuiz);

    const newTotal = oldTotal + coinsFromQuiz + bonusFromBadges;
